...
def mcluster(logger, outdir, fastadir, embeddingdir, bin_length, feature,a):
    ...
    for i in range(len(nsample)-1):
        ...
        write_bin(contig_all, labels,contig_dict,f"{outdir}",bin_length)
...
def parser_args():
    ...
    generate_data = subparsers.add_parser('generate_data', help="Generate sequence features(kmer and abundance) as training data.")
    ...
    for p in [ bin_mode, generate_data]:
        ...
        p.add_argument('--num_process', type=int, default=10, help=' Number of threads used (default: 10)')
    for p in [bin_mode, cluster]:
        p.add_argument('--evaluation',type=str, default="no_markers", help='Evaluation model used(no_markers, markers110, markers35, default: no_markers)')
    ...
    cluster.add_argument('--num_process', type=int, default=10, help=' Number of threads used (default: 10)')
    cluster.add_argument('--cuda', help = 'whether use cuda', required=False, action='store_true')
    cluster.add_argument('--batch_size', type=int, help = 'batch size (default: 64)', default=128)
    ...
    cluster.add_argument('--batchsteps', type=int, help = 'batchsteps (default: 30, 60, 120)', default=[30, 100], nargs='+')
    bin_mode.add_argument('--epoch','-n', type=int, help='training epoch (default: 300)', default=100)
    ...
    train.add_argument('--batch_size', type=int, help = 'batch size (default: 64)', default=128)
    train.add_argument('--batchsteps', type=int, help = 'batchsteps (default: 30, 60, 120)', default=[30, 100], nargs='+')
